...
(function() {
	...
	//var paramMainMessage  = TextManager.RemErrorMessage;
    ...
    Graphics._makeMainMessage = function() {
		...
		//mainMessage.innerHTML = '<hr>' + TextManager.RemErrorMessage;
		
		//Credits to @madtisa for the code
		const {versions, arch, platform} = process;
		const {nw} = versions || {};
		mainMessage.innerHTML = `<hr>${TextManager.RemErrorMessage}(${RemVersionText}, os: ${platform} (${arch}), nw: ${nw})`;
		
		let modsName = TextManager.modsNameList();
		if(modsName) mainMessage.innerHTML += '<hr>' + modsName + '<hr>';
