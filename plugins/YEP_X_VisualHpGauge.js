...
Sprite_Battler.prototype.update = function() {
	...
	if(this._battler) {
		...
		this.createVisualEnergyGaugeWindow();
	}
};
...
Sprite_Battler.prototype.createVisualHpGaugeWindow = function() {
	...
	//this._visualStaminaGauge = new Window_VisualStaminaGauge();
    //this._visualStaminaGauge.setBattler(this._battler);
    //this.parent.parent.addChild(this._visualStaminaGauge);
};
...
Sprite_Battler.prototype.setBattler = function(battler) {
	...
	if (this._visualStaminaGauge) this._visualStaminaGauge.setBattler(battler);
	if (this._visualSPGauge) this._visualSPGauge.setBattler(battler);
};
...
Window_VisualHPGauge.prototype.updateHpPosition = function() {
    ...
    if (this._currentHpValue !== this._battler.hp) {
      ...
      const difference = Math.abs(this._displayedValue - this._battler.hp);
...
Window_VisualHPGauge.prototype.updateDisplayCounter = function() {
    const targetValue = this._currentHpValue;
    if (this._battler._barrierAltered) {
      this._battler._barrierAltered = false;
    } else if (targetValue === this._displayedValue) {
      return;
    }
...
Window_VisualHPGauge.prototype.drawActorHp = function(actor, x, y, width) {
    ...
    if (Yanfly.Param.VHGShowHP) {
        ...
        const labelSprite = this.drawText(TextManager.hpA, x, y, 44);
