const MOG_PHASE_NUMBER_NAME = 'Wave_Number';
const MOG_PHASE_NUMBER_X = -50;
const MOG_PHASE_NUMBER_Y = -18;

const MOG_WAVE_NUMBER_DURATION = 4;
...
/*:
 * @plugindesc (v1.0) Activates the system of consecutive battles.
 * @author Moghunter
 *
 ...
 * =============================================================================
 * Activates the system of consecutive battles.
 *
 * =============================================================================
 * PLUGIN COMMAND
 * =============================================================================
 * Use the command below to define consecutive battles,
 *
 ...
 *
 * =============================================================================
 * To display or hide the Wave number use the commands below.
 *
 * hide_wave_number
 *
 * show_wave_number
 *
 * =============================================================================
 * To show or hide the animation of the phases use the commands below.
 *
...
　　var Moghunter = Moghunter || {};
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_ConsecutiveBattles');
	...
	Moghunter.consBat_SpriteTurnNumberX = MOG_PHASE_NUMBER_X;
	Moghunter.consBat_SpriteTurnNumberY = MOG_PHASE_NUMBER_Y;
...
Spriteset_Battle.prototype.createConBatField = function() {
	...
	this._conBatField.name = 'Battlefield';
...
Spriteset_Battle.prototype.update = function () {
	...
	if ($gameSystem._consBatWait > 0) {
		$gameSystem._consBatWait -= 60 * SceneManager._deltaTime;
	}
...
Spriteset_Battle.prototype.createEnemiesConBat = function() {
    ...
    for (var i = 0; i < enemies.length; i++) {
		...
		sprites[i].name = 'Enemy ' + enemies[i].id;
    }
...
Scene_Battle.prototype.start = function() {
	...
	&& ($gameVariables.value(VARIABLE_MOG_TURNS_SHOWED_1_ID) < $gameVariables.value(VARIABLE_MOG_TURNS_SHOWED_2_ID))
	) {
	    $gameSystem._consBaPhase = [true,true];
		$gameVariables.setValue(VARIABLE_MOG_TURNS_SHOWED_1_ID, $gameVariables.value(VARIABLE_MOG_TURNS_SHOWED_2_ID));
		//this._stateIconWindows = false;
		//this._statusWindow.refreshStateIconWindows();
	};
};
//==============================
// * create Spriteset
...
Scene_Battle.prototype.createTimeNSprite = function() {
    this._timeNCursor = new TimeNumber();
	this._timeNCursor.mz = 116;
	this._hudField.addChild(this._timeNCursor);
...
Scene_Battle.prototype.updateBattleProcess = function() {
	if (this.needSkipBattleProcessCB()) {
		$gameSystem._consBatime -= 60 * SceneManager._deltaTime;
		return;
	}
...
BattleManager.prepareConBat = function() {
	...
	SceneManager._scene._stateIconWindows = false;
};
...
BattleManager.getDataRewardsCB = function() {
	...
	$gameSystem._consBat.rewards[index].exp = 0;
	$gameSystem._consBat.rewards[index].items = [];
	$gameSystem._consBat.rewards[index].itemChance = $gameTroop.itemChanceTotal();
...
TimeNumber.prototype = Object.create(Sprite.prototype);
TimeNumber.prototype.constructor = TimeNumber;
...
TimeNumber.prototype.initialize = function() {
    Sprite.prototype.initialize.call(this);
	this.setup();
	this.createSprites();
};
...
TimeNumber.prototype.setup = function() {
	this._minuteValue = $gameParty.getTimeMinutesNumber();
	this._secondValue = '' + $gameParty.getTimeSecondsNumber();
	this._secondValue = this._secondValue.padStart(2, "0");
	this._showTimer = $gameParty._showTopRightTimeNumberFlag;
	this.opacity = 0;
};
...
WaveNumber.prototype.createSprites = function() {
	let showTopRightWave = $gameParty._showTopRightWaveNumberFlag;
	if(showTopRightWave) {
		this.createLayout();
		this.createNumber();
	}
...
TimeNumber.prototype.createSprites = function() {
...
WaveNumber.prototype.createLayout = function() {
	let imageName = "Wave_A";
	if(TextManager.isRussian) imageName += "_RU";
	else if(TextManager.isSpanish) imageName += "_SP";
	else if(TextManager.isKorean) imageName += "_KR";
	else if(TextManager.isTChinese) imageName += "_TCH";
	else if(TextManager.isSChinese) imageName += "_SCH";
	this._layout = new Sprite(ImageManager.loadSystem(imageName));
	this._layout.name = 'Layout ' + imageName;
	this._layout.x = REM_WAVE_LAYOUT_X;
	this._layout.y = REM_WAVE_LAYOUT_Y;
	this.addChild(this._layout);
};

TimeNumber.prototype.createLayout = function() {
	let imageName = "Time_A";
	if(TextManager.isRussian) imageName += "_RU";
	else if(TextManager.isSpanish) imageName += "_SP";
	else if(TextManager.isKorean) imageName += "_KR";
	else if(TextManager.isTChinese) imageName += "_TCH";
	...
	this._layout = new Sprite(ImageManager.loadSystem(imageName));
	this._layout.name = 'Layout ' + imageName;
	if($gameParty._showTopRightWaveNumberFlag && $gameSystem._consBaTurnVisible && $gameSystem._consBat.battles.length > 0) {
		this._layout.x = REM_TIMER_LAYOUT_X;
		this._layout.y = REM_TIMER_LAYOUT_Y;
	}
	else {
		this._layout.x = REM_TIMER_LAYOUT_X;
		this._layout.y = REM_WAVE_LAYOUT_Y;
...
WaveNumber.prototype.createNumber = function() {
	...
	this._number.bitmap.fontSize = REM_WAVE_FONT_SIZE;
	this._number.bitmap.fontItalic = String(Moghunter.consBat_SpriteWaveNumberFontItalic) === "true" ? true : false;
	this._number.x = this._layout.x + REM_WAVE_NUMBER_X;
	this._number.y = this._layout.y + REM_WAVE_NUMBER_Y;

	 this.addChild(this._number)
	 this.refreshWaveNumber();
};

TimeNumber.prototype.createNumber = function() {
     this._number = new Sprite(new Bitmap(160,48));

	this._number.name = 'Number';
	this._number.bitmap.fontSize = REM_TIMER_FONT_SIZE;
	this._number.bitmap.fontItalic = String(Moghunter.consBat_SpriteWaveNumberFontItalic) === "true" ? true : false;
	this._number.x = this._layout.x + REM_TIMER_NUMBER_X;
	this._number.y = this._layout.y + REM_TIMER_NUMBER_Y;
...
WaveNumber.prototype.refreshWaveNumber = function() {
	let showTopRightWave = $gameParty._showTopRightWaveNumberFlag;
	 ...
	 if(Prison.isEndlessBattle()) text = String(wave);
	 if(showTopRightWave)
		this._number.bitmap.drawText(text,0,0,this._number.width - 5,this._number.height - 5,"center");
	 $gameVariables.setValue(VARIABLE_MOG_TURNS_SHOWED_2_ID, wave);
};
...
TimeNumber.prototype.refreshTimeNumber = function() {
	...
	this._secondValue = '' + $gameParty.getTimeSecondsNumber();
	this._secondValue = this._secondValue.padStart(2, "0");
	let text = String(this._minuteValue + " : " + this._secondValue);
	if(showTopRightTimer)
		this._number.bitmap.drawText(text,0,0,this._number.width - 5,this._number.height - 5,"center");
};
...
WaveNumber.prototype.createTurnLayout = function() {
	let imageName = "Wave_B";
	if(TextManager.isRussian) imageName += "_RU";
	else if(TextManager.isSpanish) imageName += "_SP";
	else if(TextManager.isKorean) imageName += "_KR";
	else if(TextManager.isTChinese) imageName += "_TCH";
	else if(TextManager.isSChinese) imageName += "_SCH";
    this._layoutTurn = new Sprite(ImageManager.loadSystem(imageName));
	this._layoutTurn.name = 'Turn Layout';
...
WaveNumber.prototype.createTurnNumber = function() {
   ...
   */
   this._numberTurn = new Sprite_RemNumber(MOG_PHASE_NUMBER_NAME);
	this._numberTurn.name = 'Turn Number';
	this._numberTurn.x = MOG_PHASE_NUMBER_X;
	this._numberTurn.y = MOG_PHASE_NUMBER_Y;
	this._numberTurn.org = [this._numberTurn.x,this._numberTurn.y];
	this._numberTurn.setNumber(this._waveIndex + 1);
...
TimeNumber.prototype.needFade = function() {
	//if ($gameMessage.isBusy()) {return true};
	if(this._minuteValue === 0 && this._secondValue == '00') {return};
	return false;
};
...
TimeNumber.prototype.updateVisible = function() {
	const delta = 10 * 60 * SceneManager._deltaTime
	if (this.needFade()) {
	    this.opacity -= delta;
	} else {
...
WaveNumber.prototype.updateSlide = function() {
	...
	switch (this._layoutTurnPhase[0]) {
		case 0:
			if (this._layoutTurn.x < this._layoutTurn.org[0]) {
				this._layoutTurn.x += delta;
				this._layoutTurn.opacity += 5 * delta;
				if (this._layoutTurn.x >= this._layoutTurn.org[0]) {
					...
					this._layoutTurnPhase = [1, MOG_WAVE_NUMBER_DURATION];
					$gameSwitches.setValue(SWITCH_HIDE_STATE_ICONS_ID, false);
				}
			}
			break;
		case 1:
			this._layoutTurnPhase[1] -= delta;
...
TimeNumber.prototype.update = function() {
    Sprite.prototype.update.call(this);
	this.updateVisible();
	if (this.needRefreshTimeNumber()) {this.refreshTimeNumber()};
};
