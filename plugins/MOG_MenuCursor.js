...
Game_Temp.prototype.initialize = function() {
	...
	/**
	 * Elements:
	 * - 0-th - is cursor initialized,
	 * - 1-st - cursor time counter,
	 * - 2-nd - X position,
	 * - 3-rd - Y position
...
Scene_Base.prototype.create_menucursor = function() {
	...
	this._sprite_mcursor.name = 'Menu Cursor';
	this._sprite_mcursor.opacity = 0;
	this._sprite_mcursor.anchor.x = 0.5;
	this._sprite_mcursor.anchor.y = 0.5;
	/**
	 * Cursor data:
	 * - 6 - Is cursor frame animated
	 * - 7 - Frame width
	 * - 8 - Frame height
	 * - 9 - Current frame number
	 * - 10 - Time counter to change frames
	 * - 11 - Frame duration
	 * @type {[number,number,number,number,number,boolean,boolean,number,number,number,number,number,number]}
	 * @private
	 */
...
Scene_Base.prototype.update_mcursor = function () {
	...
	if (this.isBusy()) {
		this._sprite_mcursor.opacity -= 25 * delta;
		return;
	}

	if (this._mcursor_data[0] === -1) {
		this.set_mcursor_initial();
		return;
	}

	if ($gameTemp._mcursorData[1] > 0) {
		$gameTemp._mcursorData[1] -= delta;
		this._sprite_mcursor.opacity += 15 * delta;
	} else {
		this._sprite_mcursor.opacity = 0;
	}

	if (this._mcursor_data[5]) {
		this.update_mcursor_float();
	}

	if (this._mcursor_data[6]) {
		this.update_mcursor_frames();
	}

	const nx = $gameTemp._mcursorData[2] + this._mcursor_data[0] + (Math.sin(this._mcursor_data[3]) + 1) * 5;
	this._sprite_mcursor.x = this.cursorMoveToSVD(this._sprite_mcursor.x, nx, 5);
	this._sprite_mcursor.y =
		this.cursorMoveToSVD(this._sprite_mcursor.y, $gameTemp._mcursorData[3] + this._mcursor_data[1], 5);
};
...
Scene_Base.prototype.update_mcursor_frames = function() {
	this._mcursor_data[10] += 60 * SceneManager._deltaTime;
	if (this._mcursor_data[10] < this._mcursor_data[11]) {
		return;
	}
	this._mcursor_data[10] = 0;

	this._sprite_mcursor.setFrame(
		this._mcursor_data[7] * this._mcursor_data[9],
		0,
		this._mcursor_data[7],
		this._mcursor_data[8]
	);

	this._mcursor_data[9]++;
	if (this._mcursor_data[9] >= Moghunter.mcursor_maxframes) {
		this._mcursor_data[9] = 0;
	}
};
...
Scene_Base.prototype.cursorMoveToSVD = function (value, real_value, speed) {
	...
	var dnspeed = (255 + (Math.abs(value - real_value) / 255)) * 60 * SceneManager._deltaTime;
	if (value > real_value) {
		value = Math.max(real_value, value - dnspeed);
	} else if (value < real_value) {
		value = Math.min(real_value, value + dnspeed);
	}
...
Window_Selectable.prototype.set_mcursor_data = function () {
	if (!this._refCursorIndex) {
		this._refCursorIndex = true;
		this.select(this.index());
	}
	const rect = this.itemRect(this.index());
	if (rect.x < 0 || rect.y < 0 || rect.x > (this.width - 48) || rect.y > (this.height - 48)) {
		return;
	}
