...
Sprite_Doodad.prototype.updateFrame = function() {
  ...
  if (this._currentCount > 0) return this._currentCount -= 60 * SceneManager._deltaTime;
...
Window_Base.prototype.updateOpen = function() {
  if (this.isGFD() && this._opening) {
    ...
    this.opacity += 32 * delta;
    this.backOpacity += 24 * delta;
    this.contentsOpacity += 32 * delta;
...
Window_GFD_List.prototype.drawDoodadImage = function (index, filename) {
  const bitmap = ImageManager.loadDoodad(this.folderPath() + filename, 0, true);
  const sprite = new Sprite(bitmap);
  sprite.name = `Doodad ${index} ${filename}`;
  ...
  sprite.bitmap.addLoadListener((bitmap) => {
    ...
    sprite.texture.frame = new Rectangle(0, 0, pw, ph);
    sprite.position.set(dx, dy);
    sprite.scale.set(dw / pw, dh / ph);
  });
};
...
Window_GFD_SettingsHue.prototype.drawDoodadImage = function(index) {
  ...
  const bitmap = ImageManager.loadDoodad(path, hue, true);
  const sprite = new Sprite(bitmap);
  sprite.name = `Doodad ${index}`;
  this._windowContentsSprite.addChild(sprite);
