...
Sprite_ImprovedBattleback.prototype.updateFadeOut = function() {
  if (this._fadeOutDuration > 0) {
    ...
    this._fadeOutDuration = Math.max(0, this._fadeOutDuration - deltaFrames);
    if (this.opacity > 0) {
      const d = this._fadeOutDuration;
      this.opacity = Utils.applyEasing(this.opacity, 0, d);
    }
  }
};
...
Sprite_ImprovedBattleback.prototype.updateFadeIn = function() {
  if (this._fadeInDuration > 0) {
    const deltaFrames = 60 * SceneManager._deltaTime;
    this._fadeInDuration = Math.max(0, this._fadeInDuration - deltaFrames);
    if (this.opacity < 255) {
      const d = this._fadeInDuration;
      this.opacity = Utils.applyEasing(this.opacity, 255, d);
    }
  }
};
...
Spriteset_Battle.prototype.updateBattlebackBitmaps = function() {
  if (this._back1Sprite && this._back1Sprite.bitmap) {
    if (this._back1Sprite.bitmap.width <= 0) {
      this._back1Sprite.bitmap = this.battleback1Bitmap();
    }
  }
  if (this._back2Sprite && this._back2Sprite.bitmap) {
    if (this._back2Sprite.bitmap.width <= 0) {
