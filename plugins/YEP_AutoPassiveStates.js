...
Game_BattlerBase.prototype.getPassiveStateData = function(obj) {
    ...
    let array = [];
    for(let i = 0; i < obj.passiveStates.length; ++i) {
      let stateId = obj.passiveStates[i];
...
Game_BattlerBase.prototype.isPassiveStateAffected = function(stateId) {
    if(!this.passiveStatesRaw()) return false;
...
Game_Actor.prototype.passiveStatesRaw = function() {
    ...
    let array = Game_BattlerBase.prototype.passiveStatesRaw.call(this);
    ...
    for (let i = 0; i < this._skills.length; ++i) {
      let skill = $dataSkills[this._skills[i]];
