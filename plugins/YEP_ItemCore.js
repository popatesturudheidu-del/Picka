...
DataManager.saveGameWithoutRescue = function(savefileId) {
    const saveData = this.makeSaveContents();
    const json = JsonEx.stringify(saveData);
    StorageManager.save(savefileId, json);
    this._lastAccessedId = savefileId;
    const globalInfo = this.loadGlobalInfo();
    globalInfo[savefileId] = this.makeSavefileInfo(saveData);
...
Game_Actor.prototype.hasBaseItem = function(baseItem) {
    ...
    let type = (DataManager.isWeapon(baseItem)) ? 'weapon' : 'armor';
...
Window_ItemList.prototype.listEquippedItems = function() {
    ...
    let results = [];
    for (let a = 0; a < $gameParty.members().length; ++a) {
		let actor = $gameParty.members()[a];
		if (!actor) continue;
...
Window_ShopStatus.prototype.drawIndependentPossession = function(x, y) {
    ...
    this.resetTextColor();
    const possessionSprite = this.drawText(value, x, y, width, 'right');
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.possession, x, y, width - Utils.getActualTextWidth(possessionSprite));
};
...
Window_ItemStatus.prototype.drawLargeIcon = function() {
    ...
    const scaleX = Yanfly.Param.ItemIconSize / Window_Base._iconWidth;
    const scaleY = Yanfly.Param.ItemIconSize / Window_Base._iconHeight;
    const x = Window_Base._faceWidth / 2;
    const y = Window_Base._faceHeight / 2;
    const sprite = this.getIconFromSet('IconSet', iconIndex, dx, dy);
    sprite.name = `Large Icon ${iconIndex}`;
