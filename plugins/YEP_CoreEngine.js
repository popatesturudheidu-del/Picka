...
var Yanfly = Yanfly || {};
Yanfly._logger = Logger.createDefaultLogger('yep-core');
...
Scene_Title.prototype.rescaleTitleSprite = function(sprite) {
  sprite.bitmap.addLoadListener((bitmap) => {
    ...
    var ratioX = width / bitmap.width;
    var ratioY = height / bitmap.height;
...
Sprite_Battleback.prototype.scaleSprite = function() {
  this.bitmap.addLoadListener((bitmap) => {
    ...
    if (this.bitmap.width < width) {
      this.scale.x = width / bitmap.width;
    }
    if (this.bitmap.height < height) {
      this.scale.y = height / bitmap.height;
    }
...
Window_Base.prototype.textWidthEx = function(text) {
  console.warn('Avoid using textWidthEx. Use parseText or calculate text width without drawing');
  const textSprite = this.parseText(text);
  const width = textSprite.logicalWidth;
  textSprite.destroy();
  return width;
};
...
Window_Base.prototype.drawActorLevel = function(actor, x, y) {
    ...
    const labelSprite = this.drawText(TextManager.levelA, x, y, this.width - x);
    const labelWidth = Utils.getActualTextWidth(labelSprite);
    this.resetTextColor();
    var level = Yanfly.Util.toGroup(actor.level);
    var dw2 = this.textWidth(Yanfly.Util.toGroup(actor.maxLevel()));
    this.drawText(level, x + labelWidth, y, dw2, 'right');
};
...
Window_BattleStatus.prototype.drawBasicArea = function(rect, actor) {
    ...
    this.drawActorName(actor, rect.x, rect.y, nameWidth);
...
//Credits to @madtisa for the optimization!
Yanfly.Util.toGroup = function(inVal) {
	if (this.isDigitGroupingEnabled === undefined) {
		this.isDigitGroupingEnabled = eval(Yanfly.Param.DigitGroup);
	}
  if (typeof inVal !== 'string') { inVal = String(inVal); }
  //if (!eval(Yanfly.Param.DigitGroup)) return inVal;
	if (!this.isDigitGroupingEnabled) {
		return inVal;
	}
 ...
 * @param {string} code - executed code caused the exception
...
Yanfly.Util.displayError = function(e, code, message) {
  ...
  //console.error(e);
  Yanfly._logger.error({code, err: e}, message ?? e?.message);


  if (Utils.isNwjs() && Utils.isOptionValid('test')) {
    if (require('nw.gui').Window.get().isDevToolsOpen && !require('nw.gui').Window.get().isDevToolsOpen()) {
