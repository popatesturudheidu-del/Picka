...
(function() {
  ...
  Game_CharacterBase.prototype.updateTone = function() {
    ...
    if (this._toneDuration > 0) {
      ...
      this._toneDuration = Math.max(0, this._toneDuration - deltaFrames);
      const d = this._toneDuration;
      for (let i = 0; i < 4; i++) {
        this._tone[i] = Utils.applyEasing(this._tone[i], this._toneTarget[i], d);
      }
    }
  };
  ...
  Game_CharacterBase.prototype.updateOpacity = function() {
    ...
    if (this._opacityDuration > 0) {
      const deltaFrames = 60 * SceneManager._deltaTime;
      this._opacityDuration = Math.max(0, this._opacityDuration - deltaFrames);
      const d = this._opacityDuration;
      this._opacity = Utils.applyEasing(this._opacity, this._opacityTarget, d);
    }
  };
  ...
  Sprite_Character.prototype.updatePosition = function() {
    ...
    const multiplier = 60 * SceneManager._deltaTime;
    this.x += this._character._offsetX * multiplier;
    this.y += this._character._offsetY * multiplier;
    ...
    if (this._character._offsets2) {
      ...
      this.x += (offsets2.x / 2 + frameOffsets.x) * multiplier;
      this.x -= (this.patternWidth() - frameOffsets.width) / 2 * multiplier;
      this.y -= Math.max(this.patternHeight() - sry, -offsets2.y, 0) * multiplier;
    }
  };
  ...
  Game_CharacterBase.prototype.makeCrossFadeSpriteRequested = function () {
    return this._IgDuration > 0 && !this._hasChildSprite;
  };
  ...
  Game_CharacterBase.prototype.updateImageGrid = function () {
    if (this._IgDuration > 0) {
      ...
      const isInitialFrame = this._IgDuration - this._IgFrames === 1;
      if (isInitialFrame) {
        this.setupNewImageGrid();
      }

      const deltaFrames = 60 * SceneManager._deltaTime;
      this._IgDuration = Math.max(0, this._IgDuration - deltaFrames);
      const igCount = this._IgFrames - this._IgDuration;
      if (igCount >= 0) {
        this._opacity = 255.0 * igCount / this._IgFrames;
      }
    }
  };
  ...
  Sprite_CrossFade.prototype.initialize = function(character) {
   ...
   Sprite_CrossFade.prototype.update = function() {
    ...
    this.updatePosition();
    if (this._character._hasChildSprite && this._character._IgDuration === 0) {
