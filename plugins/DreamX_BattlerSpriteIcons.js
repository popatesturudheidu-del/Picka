...
//    };
    ...
    Scene_Battle.prototype.addBattlerStateWindows = function () {
		...
		if(this._stateIconLayer) {
			this._spriteset.removeChild(this._stateIconLayer);
		}

        this._stateIconLayer = new Sprite();
        this._stateIconLayer.name = 'State Icons Layer';
		...
		for(let battler of battlers) {
			...
			*/
            ...
            let normalWindow = new Window_BattleNormalStateIcon(battler);
    ...
    Scene_Base.prototype.addToolTipWindow = function () {
        if (DreamX.Param.BSIShowToolTips) {
            ...
            this._stateToolTipLayer.name = 'State ToolTips Layer';
    ...
    Window_BattleStateIcon.prototype.update = function () {
		...
		let switchId = SWITCH_HIDE_STATE_ICONS_ID;
		let invalidShow = (Karryn.isInShowEnemyGaugeOnlyDuringValidSelectionPose() && (!SceneManager._scene._enemyWindow.isOpenAndActive() || !this.battler()._selectionShowName)) ||
		(this.battler()._tagDontDrawGauge || Karryn.isInDontShowEnemyStateIconsPose());
		if(this.battler()._tagAlwaysShowStates) invalidShow = false;
        if ($gameSwitches.value(switchId) || invalidShow) {
            this.hide();
        } else {
            this.show();
        }
	...
	Window_BattleStateIcon.prototype.isMouseOverStates = function() {
        ...
        for(let state of states) {
            if(state.iconIndex > 0) stateIconNum++;
		}
    ...
    Window_BattleStateIcon.prototype.updateWindowPosition = function () {
		...
		//REM
		let drawHalfWidth = Karryn.isInDrawEnemiesAtHalfWidthPose();
		let drawHalfWidthException = this._battler.isBarTableType;
		let rowHeight = this._battler.enemy().dataRowHeight;
		if(drawHalfWidth && !drawHalfWidthException) {
			this.x = this._battler.spritePosX() + ENEMY_NAME_HALFWIDTH_STATES_ICON_X;
			this.y = this._battler.spritePosY() + ENEMY_NAME_STATES_ICON_Y + ENEMY_NAME_Y_VARIABLE;
		}
		else if(Karryn.isInReceptionistPose()) {
			this.x = this._battler.spritePosX() + VISITOR_STATES_ICON_X;
			this.y = this._battler.spritePosY() + ENEMY_NAME_STATES_ICON_Y + ENEMY_NAME_Y_VARIABLE;
		}
		else if(rowHeight === 1) {
			this.x = this._battler.spritePosX() + ENEMY_NAME_STATES_ICON_X;
			if(this._battler.spritePosY() === 0)
				this.y = -200;
			else
				this.y = this._battler.spritePosY() + ENEMY_NAME_STATES_ICON_Y + ENEMY_NAME_Y_VARIABLE;
			...
			this.x = this._battler.spritePosX() + ENEMY_NAME_SECOND_STATES_ICON_X;
			this.y = this._battler.spritePosY() + ENEMY_NAME_STATES_ICON_Y + ENEMY_NAME_Y_VARIABLE + ENEMY_NAME_SECOND_STATES_ICON_Y;
		}
		else if(rowHeight === 3) {
			this.x = this._battler.spritePosX() + ENEMY_NAME_SECOND_STATES_ICON_X;
			this.y = this._battler.spritePosY() + ENEMY_NAME_STATES_ICON_Y + ENEMY_NAME_Y_VARIABLE + ENEMY_NAME_SECOND_STATES_ICON_Y;
    ...
    Window_BattleStateIcon.prototype.drawState = function (x, y, index, array) {
		...
		//Rem
		if(stateBuff.hasLanguageIcon) {
			this.drawLanguageIcon(stateBuff.languageIndex, x, y);
		}
		else {
    ...
    Window_BattleStateIcon.prototype.highestLineIconCount = function () {
        ...
        let normalIconsCount = battler.DreamX_BSI_AllNormal().length;
		/*
		...
		*/
		...
		let highestCount = normalIconsCount;
...
//=============================================================================
    ...
    Game_BattlerBase.prototype.DreamX_BSI_StateObject = function (state) {
        ...
        var obj = {icon: state.iconIndex, id: state.id,
            state: true, hasLanguageIcon: state.hasLanguageIcon, languageIndex:state.languageIconIndex};
	...
	Game_Enemy.prototype.midBattleSpawn_setupDreamX = function() {
		let normalWindow = new Window_BattleNormalStateIcon(this);
		//var badWindow = new Window_BattleBadStateIcon(this);

		SceneManager._scene._stateIconLayer.addChild(normalWindow);
		//SceneManager._scene._stateIconLayer.addChild(badWindow);

		SceneManager._scene._stateIconWindows.push(normalWindow);
		//SceneManager._scene._stateIconWindows.push(badWindow);
	};
