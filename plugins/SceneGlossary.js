...
(function() {
    ...
    Game_Party.prototype.getGlossaryHelpMessages = function() {
		...
		return [TextManager.RemGlossaryHelp, TextManager.RemGlossaryCatHelp, setting.ConfirmHelp, setting.UsingHelp];
    };
    ...
    Scene_Glossary.prototype.createBackground = function() {
        ...
        if (pictureName) {
            ...
            const sprite = new Sprite(ImageManager.loadPicture(pictureName, 0));
    ...
    Window_GlossaryList.prototype.select = function(index) {
        ...
        if (prevItem) {
            ...
            if (result) {
                this.drawItemWithContext(this._index);
            }
        }
    ...
    Window_Glossary.prototype.cursorLeft = function(wrap) {
        ...
        if (this.canMoveLeft()) {
            ...
            this.drawItemWithContext(this._maxPages - 1);
        }
    };
    ...
    Window_Glossary.prototype.getGlossaryBitmap = function(index) {
        ...
        if (pictureName) {
			...
			return ImageManager.loadEnemy(pictureName, 0);
        } else {
            if (enemy) {
    ...
    Window_Glossary.prototype.drawPlusPicture = function(pictureName, xText, yText) {
        ...
        const bitmap = ImageManager.loadEnemy(pictureName);
        if (!bitmap) {
            return;
        }

        const x = parseInt(xText) || 0;
        const y = parseInt(yText) || 0;
        const sprite = new Sprite(bitmap);
        sprite.name = `Plus Picture ${pictureName}`;
    ...
    Window_Glossary.prototype.drawPicture = function(bitmap, text, y) {
        ...
        sprite.bitmap.addLoadListener((bitmap) => {
            ...
            sprite.position.set(x, y);
            sprite.scale.set(dw / bitmap.width, dy / bitmap.height);
        });
        this._windowContentsSprite.addChild(sprite);
