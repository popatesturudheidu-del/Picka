...
Window_EquipRequirement.prototype.drawClassRequirements = function(dy) {
    ...
    this.changePaintOpacity(true);
    this.changeTextColor(this.systemColor());
    const textSprite = this.drawText(classTx, this.textPadding(), dy, ww);
    const textWidth = Utils.getActualTextWidth(textSprite);
    var dx = textWidth + this.textPadding();
    ww -= dx;
    if (style === 1 && Imported.YEP_ClassChangeCore) {
      for (var i = 0; i < length; ++i) {
        if (dx + Window_Base._iconHeight > this.contents.width) {
          dy += this.lineHeight();
          dx = textWidth + this.textPadding();
        }
...
Window_EquipRequirement.prototype.drawSkillRequirements = function(dy) {
    ...
    this.changePaintOpacity(true);
    this.changeTextColor(this.systemColor());
    const textSprite = this.drawText(skillTx, this.textPadding(), dy, ww);
    const textWidth = Utils.getActualTextWidth(textSprite);
    var dx = textWidth + this.textPadding();
    ww -= dx;
    if (style === 1) {
