...
DataManager.getParamId = function(string) {
    if (['MHP',, 'MAXHP', 'MAX HP', 'HP'].contains(string)) {
      ...
      return 1;
    } else if (['ATK', 'ATTACK', 'STR'].contains(string)) {
      return 2;
    } else if (['DEF', 'DEFENSE', 'END'].contains(string)) {
      return 3;
    } else if (['MAT', 'MAGIC ATTACK', 'M.ATTACK', 'INT', 'DEX'].contains(string)) {
      return 4;
    } else if (['MDF', 'MAGIC DEFENSE', 'M.DEFENSE', 'RES', 'MIND'].contains(string)) {
      return 5;
    } else if (['AGI', 'AGILITY', 'SPD'].contains(string)) {
      return 6;
    } else if (['LUK', 'LUK', 'CHARM', 'CHA'].contains(string)) {
      return 7;
    } else {
      return null;
    }
...
Game_Battler.prototype.paramPlus = function(paramId) {
    let value = Game_BattlerBase.prototype.paramPlus.call(this, paramId);
	/*
    const states = this.states();
    for(let state of states) {
        if (state && state.plusParams) {
            value += state.plusParams[paramId];
        }
    }
	*/
    return value;
...
Game_Battler.prototype.paramRate = function(paramId) {
	...
	const states = this.states();
    for(let state of states) {
        if (state && state.rateParams) {
            rate *= state.rateParams[paramId];
        }
    }
    return rate;
};
...
Game_Actor.prototype.paramRate = function(paramId) {
    ...
    for(let equip of equips) {
        if(equip && equip.rateParams) {
            value *= equip.rateParams[paramId];
        }
    }
    return rate;
};
...
Game_Actor.prototype.paramFlat = function(paramId) {
	...
	const equips = this.equips();
    for(let equip of equips) {
        if(equip && equip.flatParams) {
            value += equip.flatParams[paramId];
        }
    }
    return value;
};
