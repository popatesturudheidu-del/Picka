...
Game_Party.prototype.performEscapeSuccess = function() {
	//Credits to @madtisa#8743 for the for loop optimization
...
Window_EnemyVisualSelect.prototype.updateWindowSize = function () {
    const spriteWidth = this._battler.spriteWidth();
    ...
    const textWidth = this._nameTextWidth + this.textPadding() * 2;
    const width = Math.ceil(Math.max(spriteWidth, textWidth) + this.standardPadding() * 2);
    const height = Math.max(
        Math.ceil(this._battler.spriteHeight() + this.standardPadding() * 2),
...
Window_EnemyVisualSelect.prototype.updateWindowPosition = function() {
    if (!this._battler) return;
    this.x = (-this.width / 2 + this._battler.spritePosX())
        .clamp(this._minX, this._maxX);
    this.y = (-this.height + this.standardPadding() + this._battler.spritePosY())
        .clamp(this._minY, this._maxY);
};
...
Window_BattleStatus.prototype.drawCurrentAndMax = function(current, max, x, y, width, color1, color2) {
    ...
    this.drawTextFromRight(Yanfly.Util.toGroup(current), x + width, y);
};
...
Window_BattleLog.prototype.drawSimpleActionLine = function(index) {
    ...
    var rect = this.itemRectForText(index);
    this.contents.clearRect(rect.x, rect.y, rect.width, rect.height);
    const textSprite = this.drawText(text, rect.x, rect.y, Graphics.boxWidth, 'center');
    if (this._actionIcon) {
      const ix = (rect.width - Utils.getActualTextWidth(textSprite)) / 2 - 4;
