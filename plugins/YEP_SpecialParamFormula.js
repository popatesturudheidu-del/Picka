...
DataManager.getSParamId = function(string) {
    if (['TGR', 'TARGET RATE'].contains(string)) {
      return 0;
    } else if (['GRD', 'GUARD EFFECT', 'WPDEF'].contains(string)) {
      return 1;
    } else if (['REC', 'RECOVERY', 'RECOVERY RATE'].contains(string)) {
      return 2;
    } else if (['PHA', 'PHARMACOLOGY', 'ITEM EFFECT', 'ESC'].contains(string)) {
      return 3;
    } else if (['MCR', 'MP COST', 'MP COST RATE', 'WPATK'].contains(string)) {
      return 4;
    } else if (['TCR', 'TP COST', 'TP COST RATE'].contains(string)) {
      return 5;
    } else if (['PDR', 'PHYSICAL DAMAGE RATE', 'WSC'].contains(string)) {
      return 6;
    } else if (['MDR', 'MAGICAL DAMAGE RATE', 'SSC'].contains(string)) {
      return 7;
    } else if (['FDR', 'FLOOR DAMAGE RATE', 'ASC'].contains(string)) {
      return 8;
...
Game_Battler.prototype.sparamPlus = function(id) {
    let value = Game_BattlerBase.prototype.sparamPlus.call(this, id);
	/*
...
Game_Battler.prototype.sparamRate = function(id) {
    let value = Game_BattlerBase.prototype.sparamRate.call(this, id);
	const states = this.states();
    for(let state of states) {
        if(state && state.rateSParams) {
            value *= state.rateSParams[id];
        }
    }
    return value;
};
...
Game_Battler.prototype.sparamFlat = function(id) {
    let value = Game_BattlerBase.prototype.sparamFlat.call(this, id);
	/*
    var length = this.states().length;
    for (var i = 0; i < length; ++i) {
      var obj = this.states()[i];
...
Game_Actor.prototype.sparamFlat = function(id) {
    let value = Game_Battler.prototype.sparamFlat.call(this, id);
	const equips = this.equips();
    for(let equip of equips) {
        if(equip && equip.flatSParams) {
            value += equip.flatSParams[id];
        }
    }
