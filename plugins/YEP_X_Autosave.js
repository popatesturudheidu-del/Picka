...
Game_System.prototype.canAutosave = function() {
  ...
  if(ConfigManager.cheatDisableAutosave) {
	Prison.cheatMode() && $gameSelfSwitches.value([MAP_ID_KARRYN_OFFICE, 70, "A"])) return false;
  }
...
Window_Autosave.prototype.getProperties = function () {
...
Window_Autosave.prototype.initialize = function() {
  ...
  var x = MAP_AUTOSAVE_X;
  var y = MAP_AUTOSAVE_Y;
...
Window_Autosave.prototype.windowWidth = function() {
  return Graphics.boxWidth - MAP_AUTOSAVE_X;
};
...
Window_Autosave.prototype.update = function() {
  ...
  if (this._showCount > 0) {
    this._showCount -= 60 * SceneManager._deltaTime;
    this.updateFadeIn();
  } else {
    this.updateFadeOut();
  }
...
Window_Autosave.prototype.updateFadeIn = function() {
  if (this.contentsOpacity < 255) {
    this.contentsOpacity = Math.max(
        this.contentsOpacity + Yanfly.Param.AutosaveMsgFade * 60 * SceneManager._deltaTime,
        Utils.applyEasing(this.contentsOpacity, 255, this._showCount)
    );
  }
};
...
Window_Autosave.prototype.message = function() {
  if ($gameTemp._autosaveLoading) {
	...
	var iconCode = "";
	if(MAP_AUTOSAVE_ICON_ID > 0) iconCode = "\\i[" + MAP_AUTOSAVE_ICON_ID + "]";
	return iconCode + TextManager.yanflyAutosaving;
  }
};

Window_Autosave.prototype.refresh = function() {
  if ($gameTemp._autosaveLoading) return;
  this.clear();
  this.drawGradient();
  const textSprite = this.parseText(this.message());
  textSprite.position.set(this.textPadding(), 0);
  this._windowContentsSprite.addChild(textSprite);
};

Window_Autosave.prototype.drawGradient = function () {
  const height = this.lineHeight();
  const color1 = Yanfly.Param.AutosaveMsgColor1;
  const color2 = Yanfly.Param.AutosaveMsgColor2;
  const gradient = this.contents.context.createLinearGradient(0, 0, this.width, 0);
  gradient.addColorStop(0, color1);
  gradient.addColorStop(0.25, color2);
  gradient.addColorStop(1, color2);

  this.contents.context.fillStyle = gradient;
  this.contents.context.fillRect(0, 0, this.width, height);
};
