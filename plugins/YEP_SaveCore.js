...
DataManager.makeSaveContents = function() {
  ...
  var locationDisplayNameRem = DataManager.getMapLocationDisplayNameRem();
  contents.map.locationDisplayNameRem = ['',''];
  contents.map.locationDisplayNameRem[0] = locationDisplayNameRem[0];
  contents.map.locationDisplayNameRem[1] = locationDisplayNameRem[1];
  return contents;
};
*/
...
Window_Base.prototype.drawSvActor = function(actor, x, y) {
    ...
    sprite.name = `Actor Side View ${filename}`;
    ...
    bitmap.addLoadListener((bitmap) => {
        ...
        var sx = 0;
        var sy = 0;
        sprite.texture.frame = new Rectangle(sx, sy, pw, ph);
        sprite.position.set(x - pw / 2, y - ph);
    });
};
...
Window_SavefileList.prototype.drawItem = function(index) {
    ...
    this.drawIcon(icon, rect.x + 2, rect.y + 8);
...
Window_SaveAction.prototype.getCommandName = function(type) {
    switch (type) {
        case 'load':
            return TextManager.loadFileName;
        case 'save':
            return TextManager.saveFileName;
        case 'delete':
            return TextManager.deleteFileName;
        default:
            return '-';
    }
};
...
Window_SaveAction.prototype.updateHelp = function() {
    ...
    if (this.currentSymbol() === 'load') {
        text = TextManager.yanflySaveLoadHelp;
    } else if (this.currentSymbol() === 'save') {
        text = TextManager.yanflySaveSaveHelp;
    } else if (this.currentSymbol() === 'delete') {
        text = TextManager.yanflySaveDeleteHelp;
    } else {
        text = ' ';
    }
 ...
 *    gold: number
 *    difficulty: number
 *    isPrologueEnded: boolean
 *    isFreePlay: boolean
 *    isChallengeRun: boolean
 *    isDescensionRun: boolean
...
Window_SaveInfo.prototype.loadSaveInfo = function(saveId) {
    const content = StorageManager.load(saveId);
    if (!content) {
        return undefined;
    }
    const data = JsonEx.parse(content);
...
Window_SaveInfo.prototype.updateTimer = function() {
    this._waitTime -= SceneManager._deltaTime * 60;
    const saveId = this.savefileId();
    const legacySaveVersion = 121;

    try {
        const globalInfo = DataManager.loadGlobalInfo();
        let saveInfo = globalInfo[saveId];

        const isLoadingRequired = saveInfo && !saveInfo.error && !(saveInfo.infoVersion > legacySaveVersion);
        if (isLoadingRequired) {
            if (this._waitTime > 0) {
                return;
            } else {
                DataManager.reconstructGlobalSaveInfo(globalInfo, saveId);
                DataManager.saveGlobalInfo(globalInfo)
                saveInfo = globalInfo[saveId];
            }
        }

        if (saveInfo?.error) {
            this._valid = false;
            this._error = saveInfo.error;
        } else if (saveInfo && !DataManager.isThisGameFile(saveId)) {
            this._valid = false;
            this._error = TextManager.yanflySaveInvalidText;
        } else {
            this._info = saveInfo;
            this._valid = Boolean(this._info);
            if (!this._valid) {
                this._error = TextManager.yanflySaveEmpty;
            }
        }
    } catch (error) {
        this._valid = false;
        this._error = 'Unable to open save:\n' + error.message;
...
Window_SaveInfo.prototype.drawGameTitle = function(dy) {
  if (!Yanfly.Param.SaveInfoTitle || !this._info?.title) {
      return dy;
  }
...
Window_SaveInfo.prototype.drawInvalidText = function(dy) {
    ...
    this.drawText(this._error, 0, dy, this.contents.width, 'center');
};
...
Window_SaveInfo.prototype.drawPartyGraphics = function (dy) {
    ...
    const {characters, faces} = this._info;
    dy = eval(Yanfly.Param.SaveInfoPartyY);
    const actorsCount = Math.min(characters.length, faces.length);
    const dw = Math.floor(this.contents.width / actorsCount);
    let dx = Math.floor(dw / 2);

    for (let i = 0; i < actorsCount; ++i) {
        const [characterName, characterId] = characters[i];
        const [faceName, faceId] = faces[i];
        switch (Yanfly.Param.SaveInfoPartyType) {
            case 1:
                this.drawCharacter(characterName, characterId, dx, dy);
                break;
            case 2:
                const fh = Window_Base._faceHeight;
                const fw = Window_Base._faceWidth;
                const fx = dx - Math.floor(Math.min(fh, dw) / 2);
                const dif = Math.floor(Math.max(0, dw - fw) / 2);
                this.drawFace(faceName, faceId, fx - dif, dy - fh, dw, fh);
                break;
            case 3:
...
Window_SaveInfo.prototype.drawFace = function(name, index, x, y, w, h) {
	const remXPadding = -25;
    Window_Base.prototype.drawFace.call(this, name, index, x + remXPadding, y, w, h);
};
...
Window_SaveInfo.prototype.drawPartyNames = function (dy) {
    ...
    const dw = Math.floor(this.contents.width);
    let dx = 0;

    const {actorTitle} = this._info
    if (actorTitle) {
        this.drawText(actorTitle, dx, dy, dw, 'center');
        dx += dw
    }
...
Window_SaveInfo.prototype.drawPartyLevels = function (dy) {
    ...
    const {level, slutLevel} = this._info
    const dw = Math.floor(this.contents.width);
    const dx = 0;
    const fmt = Yanfly.Param.SaveInfoActorLvFmt;
    const hasSpaces = TextManager.isEnglish || TextManager.isRussian;

    let lvText = TextManager.wardenLevel;
    if (hasSpaces) {
        lvText += ' ';
    }
    lvText += level;

    if (DEBUG_MODE && slutLevel > 0) {
        lvText += '  ' + TextManager.slutLevel;
        if (hasSpaces) {
            lvText += ' ';
        }
        lvText += slutLevel;
    ...
    let tw = this.textWidthEx(lvText);
    let dif = Math.floor(Math.max(0, dw - tw) / 2);
    this.drawTextEx(lvText, dx + dif, dy);
...
Window_SaveInfo.prototype.drawColumnData = function(dy) {
    ...
    const dw = Math.floor(this.contents.width / totalColumns);
    const dif = totalColumns > 1 ? this.textPadding() : 0;
    for (var i = 0; i < totalColumns; ++i) {
        const column = drawnArrays[i];
        const dx = i * dw;
...
Window_SaveInfo.prototype.drawColumn = function (column, dx, dy, dw) {
    ...
    const {isPrologueEnded, playthroughs, gameClears} = this._info
    for (let i = 0; i < length; ++i) {
        this.resetFontSettings();
        this.contents.fontSize = Yanfly.Param.SaveInfoDataSz;
        const data = column[i].toUpperCase().trim();
        const isSkipped =
            data === 'EMPTY' ||
            (data !== 'PLAYTIME' && !this._info.isPrologueEnded) ||
            (data === 'TOTAL DAYS' && !this._info.totalDays) ||
            (data === 'TOTAL PLAYTHROUGHS' && !this._info.playthroughs) ||
            (data === 'TOTAL GAMECLEARS' && !this._info.gameClears);
...
Window_SaveInfo.prototype.drawData = function(data, dx, dy, dw) {
    ...
    } else if (data === 'PRISON LEVEL') {
        this.drawPrisonLevel(dx, dy, dw);
    } else if (data === 'DIFFICULTY') {
        this.drawDifficulty(dx, dy, dw);
    } else if (data === 'PLAYTIME') {
        this.drawPlaytime(dx, dy, dw);
    } else if (data === 'SAVE COUNT') {
        this.drawSaveCount(dx, dy, dw);
    } else if (data === 'GOLD COUNT') {
        this.drawGoldCount(dx, dy, dw);
    } else if (data === 'INCOME') {
        this.drawIncome(dx, dy, dw);
    } else if (data === 'EXPENSE') {
        this.drawExpense(dx, dy, dw);
    } else if (data === 'DATE') {
        this.drawDate(dx, dy, dw);
    } else if (data === 'ORDER') {
        this.drawOrder(dx, dy, dw);
    } else if (data === 'CONTROL') {
        this.drawControl(dx, dy, dw);
    } else if (data === 'CORRUPTION') {
        this.drawCorruption(dx, dy, dw);
    } else if (data === 'TITLE COUNT') {
        this.drawTitleCount(dx, dy, dw);
    } else if (data === 'SLUT LEVEL') {
        this.drawSlutLevel(dx, dy, dw);
    } else if (data === 'TOTAL DAYS') {
        this.drawTotalDays(dx, dy, dw);
    } else if (data === 'TOTAL PLAYTHROUGHS') {
        this.drawTotalPlaythroughs(dx, dy, dw);
    } else if (data === 'TOTAL GAMECLEARS') {
        this.drawTotalGameClears(dx, dy, dw);
    } else if (data === 'ENDINGS') {
        this.drawEndings(dx, dy, dw);
...
Window_SaveInfo.prototype.drawLocation = function(dx, dy, dw) {
	const {mapId} = this._info;
    if (Yanfly.Param.SaveMapDisplayName) {
		var text = '';
		text = DataManager.getMapLocationDisplayNameRem(mapId);
      if (text.length <= 0 && $dataMapInfos[mapId]) text = $dataMapInfos[mapId].name;
    } else if ($dataMapInfos[mapId]) {
      var text = $dataMapInfos[mapId].name;
    } else {
      var text = '';
    }
...
Window_SaveInfo.prototype.drawPrisonLevel = function(dx, dy, dw) {
    const {prisonLevel} = this._info;
    const prisonLevelName = (prisonLevel > 0)
        ? TextManager.prisonMapLevelName.format(prisonLevel)
        : TextManager.prisonMapOutsideName;

    this.changeTextColor(this.systemColor());
	this.drawText(prisonLevelName, dx, dy, dw, 'center');
	this.resetTextColor();
};

Window_SaveInfo.prototype.drawDifficulty = function(dx, dy, dw) {
	const {difficulty, isFreePlay, isChallengeRun, isDescensionRun} = this._info;
    ...
    switch (difficulty) {
        case DIFFICULTY_NORMAL:
            ...
            if (isFreePlay) {
                difficultyText = TextManager.WardenFreePlayMode;
            } else if (isChallengeRun) {
                difficultyText = TextManager.WardenChallengeRun;
            } else if (isDescensionRun) {
                difficultyText = TextManager.WardenDescensionRun;
            } else {
                difficultyText = TextManager.WardenMode;
            }
            break;
        case DIFFICULTY_HARD:
            this.changeTextColor(this.textColor(18));
            if (isFreePlay) {
                difficultyText = TextManager.PrisonerFreePlayMode;
            } else if (isChallengeRun) {
                difficultyText = TextManager.PrisonerChallengeRun;
            } else if (isDescensionRun) {
                difficultyText = TextManager.PrisonerDescensionRun;
            } else {
                difficultyText = TextManager.PrisonerMode;
            }
            break;
        default:
            if (isFreePlay) {
                difficultyText = TextManager.SecretaryFreePlayMode;
            } else if (isChallengeRun) {
                difficultyText = TextManager.SecretaryChallengeRun;
            } else if (isDescensionRun) {
                difficultyText = TextManager.SecretaryDescensionRun;
            } else {
                difficultyText = TextManager.SecretaryMode;
            }
            break;
    }
    this.drawText(difficultyText, dx, dy, dw, 'center');
	this.resetTextColor();
};

Window_SaveInfo.prototype.drawPlaytime = function(dx, dy, dw) {
    const {playtime} = this._info;

    const hour = Math.floor(playtime / 3600);
    const min = Math.floor(playtime / 60) % 60;
    const sec = Math.floor(playtime) % 60;
    const formattedPlaytime = hour.padZero(2) + ':' + min.padZero(2) + ':' + sec.padZero(2);

    if (Yanfly.Param.SaveVocabPlaytime.length > 0) {
      this.changeTextColor(this.systemColor());
      this.drawText(TextManager.yanflySavePlaytime, dx, dy, dw, 'left');
      this.changeTextColor(this.normalColor());
      this.drawText(formattedPlaytime, dx, dy, dw, 'right');
    } else {
      this.drawText(formattedPlaytime, dx, dy, dw, 'center');
    }
};

Window_SaveInfo.prototype.drawSaveCount = function(dx, dy, dw) {
    const {saveCount} = this._info;
    const text = Yanfly.Util.toGroup(saveCount);
    if (Yanfly.Param.SaveVocabSaveCount.length > 0) {
        this.changeTextColor(this.systemColor());
        this.drawText(TextManager.yanflySaveTotalSaves, dx, dy, dw, 'left');
        this.changeTextColor(this.normalColor());
        this.drawText(text, dx, dy, dw, 'right');
    } else {
        this.drawText(text, dx, dy, dw, 'center');
...
Window_SaveInfo.prototype.drawGoldCount = function(dx, dy, dw) {
    const {gold} = this._info;
    const text = Yanfly.Util.toGroup(gold);
    if (Yanfly.Param.SaveVocabGoldCount.length > 0) {
        this.changeTextColor(this.systemColor());
        this.drawText(TextManager.funding + ":", dx, dy, dw, 'left');
        this.changeTextColor(this.normalColor());
        this.drawText(text + 'G', dx, dy, dw, 'right');
    } else {
        const formattedText = '\\c[0]' + text + this.systemColorEx() + TextManager.currencyUnit;
        this._drawData = true;
        const fw = this.textWidthEx(formattedText);
        dx += Math.max(0, Math.floor((dw - fw) / 2));
        this.drawTextEx(formattedText, dx, dy);
        this._drawData = false;
    }
};

Window_SaveInfo.prototype.drawIncome = function(dx, dy, dw) {
    const {income} = this._info;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.income + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText('+' + income, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawExpense = function(dx, dy, dw) {
    const {expense} = this._info;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.expense + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText('-' + expense, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawOrder = function(dx, dy, dw) {
    const {order} = this._info;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.order + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText(order, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawControl = function(dx, dy, dw) {
    const {control} = this._info;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.orderChange + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
	const controlText = (control > 0 ? '+' : '') + control;
    this.drawText(controlText, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawCorruption = function(dx, dy, dw) {
    const {corruption} = this._info;
	this.changeTextColor(this.systemColor());
    this.drawText(TextManager.corruption + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText(corruption, dx, dy, dw, 'right');
};

Window_SaveInfo.prototype.drawTitleCount = function(dx, dy, dw) {
    const {titlesCount} = this._info;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.titles + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText(titlesCount, dx, dy, dw, 'right');
};

Window_SaveInfo.prototype.drawSlutLevel = function(dx, dy, dw) {
    const {slutLevel} = this._info;
	if(!DEBUG_MODE || !slutLevel) return;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.slutLevel + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText(slutLevel, dx, dy, dw, 'right');
};

Window_SaveInfo.prototype.drawDate = function(dx, dy, dw) {
    const {date} = this._info;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.day + ":", dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText(date, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawTotalDays = function(dx, dy, dw) {
    const {totalDays} = this._info;
    if(!totalDays) return;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.yanflySaveTotalDays, dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText(totalDays, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawTotalPlaythroughs = function(dx, dy, dw) {
    const {playthroughs} = this._info;
    if(!playthroughs) return;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.yanflySaveTotalPlaythroughs, dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText(playthroughs, dx, dy, dw, 'right');
};
Window_SaveInfo.prototype.drawTotalGameClears = function(dx, dy, dw) {
	const {gameClears} = this._info;
    if(!gameClears) return;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.yanflySaveTotalGameClears, dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText(gameClears, dx, dy, dw, 'right');
};

Window_SaveInfo.prototype.drawEndings = function(dx, dy, dw) {
	const {endingsSeen} = this._info;
    if(!endingsSeen) return;
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.yanflySaveTotalEndings, dx, dy, dw, 'left');
    this.changeTextColor(this.normalColor());
    this.drawText(endingsSeen, dx, dy, dw, 'right');
};

Window_SaveInfo.prototype.drawVariable = function(id, dx, dy, dw) {
    if (!this._info.variables) return;
    const varName = $dataSystem.variables[id];
    varName = varName.replace(/<<(.*?)>>/i, '');
    const text = Yanfly.Util.toGroup(this._info.variables[id] ?? 0);
    const diff = Math.max(0, (this.standardFontSize() - this.contents.fontSize) / 2);
    if (varName.length > 0) {
        this._drawData = true;
        this.changeTextColor(this.systemColor());
        dy += diff;
...
Window_SaveConfirm.prototype.makeCommandList = function() {
    ...
    this.addCommand(TextManager.yanflySaveYes, 'confirm');
    this.addCommand(TextManager.yanflySaveNo, 'cancel');
};
...
Scene_File.prototype.createHelpWindow = function() {
	...
	this._helpWindow.setText(TextManager.yanflySaveSelectHelp);
...
Scene_File.prototype.onActionLoad = function() {
    if (Yanfly.Param.SaveConfirmLoad) {
	  ...
	  this.startConfirmWindow(TextManager.yanflySaveLoadText);
    } else {
...
Scene_File.prototype.onActionSave = function() {
  ...
  if (Yanfly.Param.SaveConfirmSave && StorageManager.exists(id)) {
	...
	this.startConfirmWindow(TextManager.yanflySaveSaveText);
  } else {
...
Scene_File.prototype.onActionDelete = function() {
    if (Yanfly.Param.SaveConfirmDel) {
	  ...
	  this.startConfirmWindow(TextManager.yanflySaveDeleteText);
    } else {
