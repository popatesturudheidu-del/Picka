...
DataManager.getXParamId = function(string) {
    if (['HIT', 'HIT RATE'].contains(string)) {
      ...
      return 3;
    } else if (['MEV', 'MAGIC EVADE', 'MAGIC EVASION', 'GRAZE'].contains(string)) {
      return 4;
...
Game_Battler.prototype.xparamPlus = function(id) {
    let value = Game_BattlerBase.prototype.xparamPlus.call(this, id);
	/*
...
Game_Battler.prototype.xparamRate = function(id) {
    let value = Game_BattlerBase.prototype.xparamRate.call(this, id);
	const states = this.states();
    for(let state of states) {
        if(state && state.rateXParams) {
            value *= state.rateXParams[id];
        }
    }
    return value;
};
...
Game_Battler.prototype.xparamFlat = function(id) {
    let value = Game_BattlerBase.prototype.xparamFlat.call(this, id);
	/*
    var length = this.states().length;
    for (var i = 0; i < length; ++i) {
      var obj = this.states()[i];
