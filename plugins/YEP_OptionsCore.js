...
Window_OptionsCategory.prototype.updateHelp = function() {
  ...
  if (this.currentExt()) {
	...
	let str = data.HelpDesc;
	if(str.includes('TextManager')) {
		str = str.slice(1, str.length-1);
		str = eval(str);
	}
	else str = JSON.parse(data.HelpDesc);
...
Window_OptionsCategory.prototype.drawItem = function(index) {
  ...
  this.resetTextColor();
  this.changePaintOpacity(this.isCommandEnabled(index));
  this.drawTextEx(eval(String(this.commandName(index))), rect.x, rect.y);
...
Window_Options.prototype.drawOptionsName = function(index) {
  ...
  let str = String(this.commandName(index));
	if(str.includes('TextManager')) {
		//str = str.slice(1, str.length-1);
		str = eval(str);
	}
...
Window_Options.prototype.updateHelp = function() {
  ...
  if (this._symbolData && this._symbolData[symbol]) {
	//console.log(this._symbolData[symbol].HelpDesc);
	  
	let str = this._symbolData[symbol].HelpDesc;
	if(str.includes('TextManager')) {
		str = str.slice(1, str.length-1);
		str = eval(str);
	}
	else str = JSON.parse(this._symbolData[symbol].HelpDesc);
