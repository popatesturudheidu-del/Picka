...
Window_Selectable.prototype.setCursorRect = function(x, y, width, height) {
	if (this._scrollBar?.visible && x + width > this._scrollBar.x - this.padding) {
		width -= this.width - this._scrollBar.x;
	}
...
SpriteScrollBar.prototype.createButton = function() {
	...
	for (var i = 0; i < 3; i++) {
		 ...
		 this._button[i].name = `Button ${i}: ${this._img2._url}`;
...
SpriteScrollBar.prototype.updateButton = function () {
	for (let i = 0; i < this._button.length; i++) {
		const by = this.buttonPos();
		let ny = 0;
		switch (i) {
			case 0:
				ny = by - this._img2.height;
				break;
			case 1:
				ny = by;
				break;
			case 2:
				ny = by + this.buttonSize() + this._img2.height;
				break;
		}
...
SpriteScrollBar.prototype.buttonPos = function() {
	...
	//Bugfix credit to @madtisa
	const maxTopRow = this._window.maxTopRow();
	var space = maxTopRow === 0 ? 0 : space_max / maxTopRow;
...
SpriteScrollBar.prototype.createBack = function() {
	...
	for (var i = 0; i < 3; i++) {
		 ...
		 this._back[i].name = `Back ${i}: ${this._img1._url}`;
...
SpriteScrollBar.prototype.isVisible = function() {
	...
	if(this._window._logMainSprite && this._window._logMainSprite.visible) return false;
	return true;
};
this._logMainSprite
//==============================
