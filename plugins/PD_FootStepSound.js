...
(function(){
    ...
    Game_Player.prototype.update = function(sceneActive) {
        ...
        if(this._prevpattern != this.pattern() && !this.isInVehicle()){
			...
			if($gameSwitches.value(SWITCH_FOOTSTEPS_SE_OFF_ID) || $gameActors.actor(ACTOR_KARRYN_ID).characterName().includes('C_Extra1')) {
				
			}
            else if(this.pattern() != 1) {
				let footstepFileName = fileName;
				if(!Karryn.isWearingGlovesAndHat() && (id === 0 || id === 3)) footstepFileName += 'naked_'
				footstepFileName += id;
                var seObj = {"name":footstepFileName,"volume":90,"pitch":100 + (10 * this.pattern()),"pan":0};
