...
var REM_ENEMY_SKILLID_MASTURBATE = 42;
var REM_ENEMY_SKILLID_ORGASM = 69;
...
Game_Enemy.prototype.skills = function() {
  ...
  skills = AIManager.getPatternSkills(skills, this.enemy().aiPattern, this.enemy());
  return skills;
};
...
AIManager.isDecidedActionAI = function(line) {
	...
	if(this._aiSkillId === REM_ENEMY_SKILLID_MASTURBATE) {
		this._aiSkillId = this._battler.enemy().dataMasturbateSkillId;
	}
	else if (this._aiSkillId === REM_ENEMY_SKILLID_ORGASM) {
		this._aiSkillId = this._battler.enemy().dataOrgasmSkillId;
	}
...
AIManager.getPatternSkills = function(array, patterns, enemy) {
	...
	if(skillId === REM_ENEMY_SKILLID_MASTURBATE) {
		skillId = enemy.dataMasturbateSkillId;
	}
	else if (skillId === REM_ENEMY_SKILLID_ORGASM) {
		skillId = enemy.dataOrgasmSkillId;
	}
...
AIManager.passAIConditions = function(line) {
	...
	// GROUP FEMALE MEMBERS EVAL
    if (line.match(/(.*)[ ]FEMALE[ ]MEMBERS[ ](.*)/i)) {
      var members = String(RegExp.$1);
      var condition = String(RegExp.$2);
      return this.conditionGroupFemale(members, condition);
    }
	// GROUP SELF AROUSED+FEMALE MEMBERS EVAL
    if (line.match(/(.*)[ ]SELF_AROUSED_FEMALE[ ]MEMBERS[ ](.*)/i)) {
      var members = String(RegExp.$1);
      var condition = String(RegExp.$2);
      return this.conditionGroupSelfArousedFemale(members, condition);
    }
	// GROUP SELF ORGASM POINT+FEMALE MEMBERS EVAL
    if (line.match(/(.*)[ ]SELF_ORGASM_FEMALE[ ]MEMBERS[ ](.*)/i)) {
      var members = String(RegExp.$1);
      var condition = String(RegExp.$2);
      return this.conditionGroupSelfOrgasmFemale(members, condition);
    }
	// GROUP MALE MEMBERS EVAL
    if (line.match(/(.*)[ ]MALE[ ]MEMBERS[ ](.*)/i)) {
      var members = String(RegExp.$1);
      var condition = String(RegExp.$2);
      return this.conditionGroupMale(members, condition);
    }
	// GROUP SELF AROUSED+MALE MEMBERS EVAL
    if (line.match(/(.*)[ ]SELF_AROUSED_MALE[ ]MEMBERS[ ](.*)/i)) {
      var members = String(RegExp.$1);
      var condition = String(RegExp.$2);
      return this.conditionGroupSelfArousedMale(members, condition);
    }
	// GROUP SELF ORGASM POINT+MALE MEMBERS EVAL
    if (line.match(/(.*)[ ]SELF_AROUSED_MALE[ ]MEMBERS[ ](.*)/i)) {
      var members = String(RegExp.$1);
      var condition = String(RegExp.$2);
      return this.conditionGroupSelfOrgasmMale(members, condition);
    }
    // USER PARAM EVAL
...
AIManager.conditionGroupFemale = function(members, condition) {
    var action = this.action();
    var item = action.item();
    var user = this.battler();
    var s = $gameSwitches._data;
    ...
    members = members.toUpperCase();
    if (['TROOP', 'TROOPS', 'ENEMY', 'ENEMIES'].contains(members)) {
      members = $gameTroop.femaleMembers();
    } else if (['PARTY', 'PLAYER'].contains(members)) {
      members = $gameParty.femaleMembers();
    } else {
      return false;
    }
    if (members.length <= 0) return false;
    ...
    try {
      ...
      Yanfly.Util.displayError(e, condition, 'A.I. GROUP FEMALE ERROR');
      return false;
    }
    var group = this.getActionGroup();
    this.setProperTarget(group);
...
AIManager.conditionGroupSelfArousedFemale = function(members, condition) {
    var action = this.action();
    var item = action.item();
    var user = this.battler();
    var s = $gameSwitches._data;
	...
	if(user.showPreOrgasmSkill() == false) return false;
	//if(user.reachedOrgasmPoint() == false) return false;
    ...
    try {
      ...
      Yanfly.Util.displayError(e, condition, 'A.I. GROUP SELFAROUSED FEMALE ERROR');
      return false;
    }
    var group = this.getActionGroup();
    this.setProperTarget(group);
...
AIManager.conditionGroupSelfOrgasmFemale = function(members, condition) {
    var action = this.action();
    var item = action.item();
    var user = this.battler();
    var s = $gameSwitches._data;
	...
	if(user.showOrgasmSkill() == false) return false;
    if (members.length <= 0) return false;
    condition = 'members.length ' + condition;
    try {
      if (!eval(condition)) return false;
    } catch (e) {
      Yanfly.Util.displayError(e, condition, 'A.I. GROUP SELFORGASM FEMALE ERROR');
      return false;
    }
    var group = this.getActionGroup();
    this.setProperTarget(group);
...
AIManager.conditionGroupMale = function(members, condition) {
    var action = this.action();
    var item = action.item();
    var user = this.battler();
    var s = $gameSwitches._data;
    ...
    members = members.toUpperCase();
    if (['TROOP', 'TROOPS', 'ENEMY', 'ENEMIES'].contains(members)) {
      members = $gameTroop.maleMembers();
    } else if (['PARTY', 'PLAYER'].contains(members)) {
      members = $gameParty.maleMembers();
    } else {
      return false;
    }
    if (members.length <= 0) return false;
    ...
    try {
      ...
      Yanfly.Util.displayError(e, condition, 'A.I. GROUP MALE ERROR');
      return false;
    }
    var group = this.getActionGroup();
    this.setProperTarget(group);
...
AIManager.conditionGroupSelfArousedMale = function(members, condition) {
    var action = this.action();
    var item = action.item();
    var user = this.battler();
    var s = $gameSwitches._data;
	...
	if(user.isAroused() == false) return false;
    if (members.length <= 0) return false;
    condition = 'members.length ' + condition;
    try {
      if (!eval(condition)) return false;
    } catch (e) {
      Yanfly.Util.displayError(e, condition, 'A.I. GROUP SELFAROUSED_MALE ERROR');
      return false;
    }
    var group = this.getActionGroup();
    this.setProperTarget(group);
...
AIManager.conditionGroupSelfOrgasmMale = function(members, condition) {
    var action = this.action();
    var item = action.item();
    var user = this.battler();
    var s = $gameSwitches._data;
    ...
    try {
      ...
      Yanfly.Util.displayError(e, condition, 'A.I. GROUP SELFORGASM_MALE ERROR');
      return false;
    }
    var group = this.getActionGroup();
    this.setProperTarget(group);
