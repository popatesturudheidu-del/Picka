...
Sprite_BacklogScroll.prototype.createBitmap = function() {
  ...
  if (!Sprite_BacklogScroll._evaliatedCreateBitmap) {
    Sprite_BacklogScroll._evaliatedCreateBitmap = eval(
        `(backlogWindow, scrollBarColor) => { ${Yanfly.Param.MsgBacklogScrBarSpriteCode} }`
    );
  }
  Sprite_BacklogScroll._evaliatedCreateBitmap(this._target, Yanfly.Param.MsgBacklogScrBarCol);
};
...
Window_MessageBacklog.prototype.createBackgroundPicture = function() {
  ...
  this._backgroundPicture.name = 'Background';
...
Window_MessageBacklog.prototype.processTouch = function() {
  if (!this.isOpenAndActive()) return;
  this._touchHold -= 60 * SceneManager._deltaTime;
...
Window_MessageBacklog.prototype.drawItem = function(index) {
  ...
  if (symbol === 'linebreak') {
    ...
    this.drawItemWithContext(index - ext);
  }
};
