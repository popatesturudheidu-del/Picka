...
Game_Actor.prototype.gainHp = function(value) {
	...
	this._bhud_face_data[3] += 60 * SceneManager._deltaTime;
};
...
Window_ActorCommand.prototype.sprite_move_to = function (value, real_value) {
	...
	const dnspeed = (2 + (Math.abs(value - real_value) / 12)) * 60 * SceneManager._deltaTime;
	if (value > real_value) {
		value -= dnspeed;
		if (value < real_value) {
			value = real_value;
...
Window_ActorCommand.prototype.slideWindow = function(win,vmode) {
	 ...
	 if (vm) {
		 ...
		 win.contentsOpacity += 15 * 60 * SceneManager._deltaTime;
	 } else {
...
Window_ActorCommand.prototype.update = function() {
	...
	if (this._force_hide_duration > 0) {
		this._force_hide_duration -= 60 * SceneManager._deltaTime;
		this.visible = false
	};
};
...
Scene_Base.prototype.createBattleHudScreenLayout = function() {
	...
	this._screen_layout.name = 'Scene Layout';
...
Scene_Base.prototype.updateBattleHudVisible = function() {
	const delta = 10 * 60 * SceneManager._deltaTime;
	if (this.isBattleHudVisible()) {
		this._screen_layout.opacity += delta;
	} else {
		this._screen_layout.opacity -= delta;
	}
};
...
Scene_Base.prototype.createActorsF = function() {
    ...
    for (var i = 0; i < $gameParty.maxBattleMembers(); i++) {
		...
		this._actorSprites[i].name = 'Actor ' + i;
...
Scene_Battle.prototype.createAllWindows = function () {
	...
	if (String(Moghunter.bhud_com_layout) === "true") {
		...
		this._actorCommandWindow.addChildAt(this._com_layout, 0);
	...
	this._partyCommandWindow.slide = Moghunter.bhud_party_slide_x !== 0 || Moghunter.bhud_party_slide_y !== 0;
	...
	if (String(Moghunter.bhud_party_layout) === "true") {
		...
		this._partyCommandWindow.addChildAt(this._party_layout, 0);
	...
	this._helpWindow.slide = Moghunter.bhud_help_slide_x !== 0 || Moghunter.bhud_help_slide_y !== 0;
	...
	if (String(Moghunter.bhud_help_layout) === "true") {
		...
		this._helpWindow.addChildAt(this._help_layout, 0);
	...
	this._skillWindow.slide = Moghunter.bhud_skill_slide_x !== 0 || Moghunter.bhud_skill_slide_y !== 0;
	...
	if (String(Moghunter.bhud_skill_layout) === "true") {
		...
		/**
		 * @param {undefined | 'Stamina' | 'Energy' | 'Willpower' | 'Sexual' | 'Special'} skillType
		 * @return {Sprite}
		 */
		const loadSkillBattleHud = (skillType) => {
			let battleHudName = 'Layout_Skill';

			if(skillType) {
				battleHudName += '_' + skillType;
			}
			else {
				battleHudName += '_Special';
			}

			const suffix = customLanguageSuffixes[ConfigManager.remLanguage];
			if (suffix) {
				battleHudName += '_' + suffix;
			}

			const battleHudSprite = new Sprite(ImageManager.loadBHud(battleHudName));
			battleHudSprite.x = Moghunter.bhud_skill_lay_x;
			battleHudSprite.y = Moghunter.bhud_skill_lay_y;
			battleHudSprite.visible = false;

			return battleHudSprite;
		};

		this._skill_layout = loadSkillBattleHud();
		this._skillWindow.addChildAt(this._skill_layout, 0);

		//Rem
		this._skill_layout_stamina = loadSkillBattleHud('Stamina');
		this._skillWindow.addChildAt(this._skill_layout_stamina, 0);

		this._skill_layout_energy = loadSkillBattleHud('Energy');
		this._skillWindow.addChildAt(this._skill_layout_energy, 0);

		this._skill_layout_sexual = loadSkillBattleHud('Sexual');
		this._skillWindow.addChildAt(this._skill_layout_sexual, 0);

		this._skill_layout_willpower = loadSkillBattleHud('Willpower');
		this._skillWindow.addChildAt(this._skill_layout_willpower, 0);

		this._skill_layout_special = loadSkillBattleHud('Special');
		this._skillWindow.addChildAt(this._skill_layout_special, 0);
	...
	this._itemWindow.slide = Moghunter.bhud_item_slide_x !== 0 || Moghunter.bhud_item_slide_y !== 0;
	...
	if (String(Moghunter.bhud_item_layout) === "true") {
		...
		this._itemWindow.addChildAt(this._item_layout, 0);
	...
	this._actorWindow.slide = Moghunter.bhud_actor_slide_x !== 0 || Moghunter.bhud_actor_slide_y !== 0;
	...
	if (String(Moghunter.bhud_actor_layout) === "true") {
		...
		this._actorWindow.addChildAt(this._actor_layout, 0);
	...
	this._enemyWindow.slide = Moghunter.bhud_enemy_slide_x !== 0 || Moghunter.bhud_enemy_slide_y !== 0;
	...
	if (String(Moghunter.bhud_enemy_layout) === "true") {
		...
		this._enemyWindow.addChildAt(this._enemy_layout, 0);
...
Scene_Battle.prototype.slideWindow = function (win, vmode) {
	const vm = vmode ? win.active : win.visible;
	if (vm) {
		var np = [win.org[0], win.org[1]];
		win.contentsOpacity += 15 * 60 * SceneManager._deltaTime;
	} else {
...
Scene_Battle.prototype.updateLayoutWindow = function () {
	...
	if (this._skill_layout) {
		let activeLayoutSprite;
		switch (this._skillWindow._stypeId) {
			case SKILLTYPE_WILLPOWER_ID:
				activeLayoutSprite = this._skill_layout_willpower;
				break;
			case SKILLTYPE_ATTACK_ID:
				activeLayoutSprite = this._skill_layout_stamina;
				break;
			case SKILLTYPE_SEXUAL_ID:
			case SKILLTYPE_MASTURBATE_COUCH_ID:
			case SKILLTYPE_MASTURBATE_INBATTLE_ID:
			case SKILLTYPE_TOILET_ID:
				activeLayoutSprite = this._skill_layout_sexual;
				break;
			case SKILLTYPE_ENERGY_ID:
				activeLayoutSprite = this._skill_layout_energy;
				break;
			case SKILLTYPE_OFFBALANCE_ID:
			case SKILLTYPE_FALLEN_ID:
			case SKILLTYPE_DISARMED_ID:
			case SKILLTYPE_WAITRESS_ID:
			case SKILLTYPE_BARTENDER_ID:
			case SKILLTYPE_RECEPTIONIST_ID:
			case SKILLTYPE_STRIPPER_ID:
			case SKILLTYPE_INTERMISSION_ID:
			case SKILLTYPE_TRAINER_ID:
				activeLayoutSprite = this._skill_layout_special;
				break;
			default:
				activeLayoutSprite = this._skill_layout_special;
				break;
		...
		activeLayoutSprite.visible = true;
		activeLayoutSprite.opacity = this._skillWindow.contentsOpacity;
	}
...
Battle_Hud.prototype.load_img = function() {
	...
	else if(TextManager.isRussian) this._state_language_img = ImageManager.loadSystem("IconSet_Language_RU");
	else if(TextManager.isKorean) this._state_language_img = ImageManager.loadSystem("IconSet_Language_KR");
	else if(TextManager.isSpanish) this._state_language_img = ImageManager.loadSystem("IconSet_Language_SP");
	else if(TextManager.isSChinese) this._state_language_img = ImageManager.loadSystem("IconSet_Language_SCH");
	else if(TextManager.isTChinese) this._state_language_img = ImageManager.loadSystem("IconSet_Language_TCH");
	else this._state_language_img = ImageManager.loadSystem("IconSet_Language_EN");
	...
	if(TextManager.isEnglish) {
		if(DEBUG_MODE)
			...
			this._en_text_img = ImageManager.loadBHud("En_Text_Steam");
	}
	else if(TextManager.isJapanese) {
		if(DEBUG_MODE)
			this._en_text_img = ImageManager.loadBHud("Jp_Text");
		else
			this._en_text_img = ImageManager.loadBHud("Jp_Text_Steam");
	}
	else if(TextManager.isRussian) {
		if(DEBUG_MODE)
			this._en_text_img = ImageManager.loadBHud("RU_Text");
		else
			this._en_text_img = ImageManager.loadBHud("RU_Text_Steam");
	}
	else if(TextManager.isKorean) {
		if(DEBUG_MODE)
			this._en_text_img = ImageManager.loadBHud("KR_Text");
		else
			this._en_text_img = ImageManager.loadBHud("KR_Text_Steam");
	}
	else if(TextManager.isSpanish) {
		if(DEBUG_MODE)
			this._en_text_img = ImageManager.loadBHud("SP_Text");
		else
			this._en_text_img = ImageManager.loadBHud("SP_Text_Steam");
	}
	else if(TextManager.isSChinese) {
		if(DEBUG_MODE)
			this._en_text_img = ImageManager.loadBHud("SCH_Text");
		else
			this._en_text_img = ImageManager.loadBHud("SCH_Text_Steam");
	}
	else if(TextManager.isTChinese) {
		if(DEBUG_MODE)
			this._en_text_img = ImageManager.loadBHud("TCH_Text");
		else
			this._en_text_img = ImageManager.loadBHud("TCH_Text_Steam");
	}
	else {
		if(DEBUG_MODE)
			this._en_text_img = ImageManager.loadBHud("En_Text");
...
Battle_Hud.prototype.update = function() {
	...
	if (this._data_initial_ref[0] < 2) {
		this._data_initial_ref[0] += 60 * SceneManager._deltaTime;
		return;
	}
...
Battle_Hud.prototype.updateSlide = function() {
	 ...
	 if (this._slideA[0] > 0) {
		 this.visible = false;
		 this.opacity = 0;
		 this._slideA[0] -= 60 * SceneManager._deltaTime;
	     return;
	 }
	 this.visible = true;
...
// * Update visible
//==============================
Battle_Hud.prototype.update_visible = function () {
	const delta = 10 * 60 * SceneManager._deltaTime;
...
Battle_Hud.prototype.update_dif = function (value, real_value, speed) {
	...
	const dnspeed = (2 + (Math.abs(value - real_value) / speed)) * 60 * SceneManager._deltaTime;
	if (value > real_value) {
		value -= dnspeed;
		if (value < real_value) {
			value = real_value;
...
Battle_Hud.prototype.update_turn = function() {
	...
	this._turn_blink[0] += delta;

	const timeCounter = Math.floor(this._turn_blink[0]);
	if (timeCounter < 60) {
		this._turn_blink[1] += 2 * delta;
...
Battle_Hud.prototype.updateTurnZoom = function () {
	if (this._turn.vis !== this._turn.visible) {
		this._turn.vis = this._turn.visible;
		this._turn.scale.x = this._turn.scale.y = 1.5;
	}

	if (this._turn.scale.x > 1) {
		const scale = Math.max(1, this._turn.scale.x - 0.04);
		this._turn.scale.x = this._turn.scale.y = scale;
	}
};
...
Battle_Hud.prototype.create_face = function() {
	...
	this._face.name = `Actor ${this._battler._actorId} Face`;
...
Battle_Hud.prototype.update_face_animation = function () {
	if (this._battler._bhud_face_data[3] <= 0) return;
	this._battler._bhud_face_data[3] -= 60 * SceneManager._deltaTime;
	if (this._battler._bhud_face_data[3] > 0) return;
...
Battle_Hud.prototype.update_face_zoom = function() {
	if (this._battler._bhud_face_data[1] <= 0) return;

	const delta = 60 * SceneManager._deltaTime;
	this._battler._bhud_face_data[1] -= delta;

	if (this._battler._bhud_face_data[1] <= 0) {
		this._face.scale.x = 1.00;
	} else if (this._battler._bhud_face_data[1] < 20) {
		this._face.scale.x = Math.max(1, this._face.scale.x - TachieZoomAnimationStep * delta);
	} else if (this._battler._bhud_face_data[1] < 40) {
		const maxScale = 1 + .25 * TachieSelectionBattlerScaling;
		this._face.scale.x = Math.min(maxScale, this._face.scale.x + TachieZoomAnimationStep * delta);
	}
...
Battle_Hud.prototype.update_face_shake = function () {
	...
	if (!this._face_data[2]) return;

	if (this._battler._bhud_face_data[0] > 0) {
		this._battler._bhud_face_data[0] -= 1;
		this._face.x = this._pos_x + Moghunter.bhud_face_pos_x + ((Math.random() * 12) - 6);
	} else if (this._battler._bhud_face_data[0] < 0) {
		this._battler._bhud_face_data[0] += 1;
...
Battle_Hud.prototype.create_hp_meter = function() {
	...
	this._hp_meter_red.name = 'HP Meter Red';
	...
	this._hp_meter_blue.name = 'HP Meter Blue';
...
Battle_Hud.prototype.create_hp_number = function() {
	...
	for (var i = 0; i < 5; i++) {
	   ...
	   this._hp_number[i].name = 'HP Digit ' + i;
...
Battle_Hud.prototype.create_maxhp_number = function() {
	...
	for (var i = 0; i < 5; i++) {
	   ...
	   this._maxhp_number[i].name = 'Max HP Digit ' + i;
...
Battle_Hud.prototype.update_hp = function() {
	if (this._hp_meter_blue) {
		if(this._hp_flow[0]) {
		   ...
		   this._hp_flow[1] += 1.5 * 60 * SceneManager._deltaTime;
...
Battle_Hud.prototype.create_mp_meter = function() {
	...
	this._mp_meter_red.name = 'MP Meter Red';
	...
	this._mp_meter_blue.name = 'MP Meter Blue';
...
Battle_Hud.prototype.create_mp_number = function() {
	...
	for (var i = 0; i < 5; i++) {
	   ...
	   this._mp_number[i].name = 'MP Digit ' + i;
...
Battle_Hud.prototype.create_maxmp_number = function() {
	...
	for (var i = 0; i < 5; i++) {
	   ...
	   this._maxmp_number[i].name = 'Max MP Digit ' + i;
...
Battle_Hud.prototype.update_mp = function() {
	if (this._mp_meter_blue) {
		if(this._mp_flow[0]) {
		   ...
		   this._mp_flow[1] += 1.5 * 60 * SceneManager._deltaTime;
...
Battle_Hud.prototype.create_wp_meter = function() {
	...
	this._wp_meter_red.name = 'WP Meter Red';
	...
	this._wp_meter_blue.name = 'WP Meter Blue';
...
Battle_Hud.prototype.create_wp_number = function() {
	...
	for (var i = 0; i < 5; i++) {
	   ...
	   this._wp_number[i].name = 'WP Digit ' + i;
...
Battle_Hud.prototype.update_wp = function() {
	if (this._wp_meter_blue) {
		if(this._wp_flow[0]) {
		   ...
		   this._wp_flow[1] += 1.5 * 60 * SceneManager._deltaTime;
...
Battle_Hud.prototype.create_desire_meter = function() {
	...
	this._cock_meter_red.name = 'Cock Meter Red';
	...
	this._cock_meter_blue.name = 'Cock Meter Blue';
	...
	this._mouth_meter_red.name = 'Mouth Meter Red';
	...
	this._mouth_meter_blue.name = 'Mouth Meter Blue';
	...
	this._boobs_meter_red.name = 'Boobs Meter Red';
	...
	this._boobs_meter_blue.name = 'Boobs Meter Blue';
	...
	this._pussy_meter_red.name = 'Pussy Meter Red';
	...
	this._pussy_meter_blue.name = 'Pussy Meter Blue';
	...
	this._butt_meter_red.name = 'Butt Meter Red';
	...
	this._butt_meter_blue.name = 'Butt Meter Blue';
...
Battle_Hud.prototype.create_desire_number = function() {
	...
	for (var i = 0; i < 3; i++) {
	   ...
	   this._cock_number[i].name = 'Cock Digit ' + i;
	...
	for (var i = 0; i < 3; i++) {
	   ...
	   this._mouth_number[i].name = 'Mouth Digit ' + i;
	...
	for (var i = 0; i < 3; i++) {
	   ...
	   this._boobs_number[i].name = 'Boobs Digit ' + i;
	...
	for (var i = 0; i < 3; i++) {
	   ...
	   this._pussy_number[i].name = 'Pussy Digit ' + i;
	...
	for (var i = 0; i < 3; i++) {
	   ...
	   this._butt_number[i].name = 'Butt Digit ' + i;
...
Battle_Hud.prototype.create_tp_meter = function() {
	...
	this._tp_meter_red.name = 'TP Meter Red';
	...
	this._tp_meter_blue.name = 'TP Meter Blue';
...
Battle_Hud.prototype.create_tp_number = function() {
	...
	for (var i = 0; i < 5; i++) {
	   ...
	   this._tp_number[i].name = 'TP Digit ' + i;
...
Battle_Hud.prototype.create_maxtp_number = function() {
	...
	for (var i = 0; i < 5; i++) {
	   ...
	   this._maxtp_number[i].name = 'Max TP Digit ' + i;
...
Battle_Hud.prototype.update_tp = function() {
	if (this._tp_meter_blue) {
		if(this._tp_flow[0]) {
		   ...
		   this._tp_flow[1] += 1.5 * 60 * SceneManager._deltaTime;
...
Battle_Hud.prototype.update_at = function() {
	if (this._at_meter) {
		...
		if(this._at_flow[0]) {
		   ...
		   this._at_flow[1] += 1.5 * 60 * SceneManager._deltaTime;
...
Battle_Hud.prototype.create_states = function() {
	...
	this._state_icon.name = 'State Icon ' + this._state_img;
...
Battle_Hud.prototype.update_states = function() {
	this._states_data[2] += 60 * SceneManager._deltaTime;
...
Battle_Hud.prototype.create_states2 = function() {
	...
	this._state_icon.name = 'State Icon';
	this._state_icon.x = this._pos_x + REM_BHD_STATES_X;
	this._state_icon.y = this._pos_y + REM_BHD_STATES_Y;
	this._state_icon.visible = false;
	this.addChild(this._state_icon);
...
Battle_Hud.prototype.refresh_states2 = function() {
	...
	if(this._stateCounters) {
		for(let scl = 0; scl < this._stateCounters.length; scl++) {
	...
	let m = Math.min(Math.max(states.length,0), REM_BHD_STATES_MAX);
	let align = Moghunter.bhud_statesAlign;
	for(let i = 0; i < m; i++){
		let state = states[i];
		//console.log(state.id)
		if (state.iconIndex <= 0) continue;
		let iconId = state.iconIndex;
		if(state.hasLanguageIcon) {
			iconId = state.languageIconIndex;
			this._stateIcons[i] = new Sprite(this._state_language_img);
		}
		else {
			this._stateIcons[i] = new Sprite(this._state_img);
		}
		this._stateIcons[i].name = 'State Icon ' + i;
	     //var sx = icons[i] % 16 * w;
		 //var sy = Math.floor(icons[i] / 16) * w;
		 let sx = iconId % 16 * w;
		 let sy = Math.floor(iconId / 16) * w;
...
Battle_Hud.prototype.drawStateTurns = function(actor, state, wx, wy) {
  ...
  counter.name = `State ${state.id} Turns`;
...
Battle_Hud.prototype.drawStateCounter = function(actor, state, wx, wy) {
  ...
  var counter = new Sprite(new Bitmap(iconWidth,iconHeight));
  counter.name = `State ${state.id} Counter`;
  counter.bitmap.clear();
