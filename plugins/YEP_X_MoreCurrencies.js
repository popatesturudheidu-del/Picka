...
Window_Base.prototype.drawAltCurrency = function(value, unit, wx, wy, ww) {
    ...
    if (unitText !== '') {
      ...
      const textSprite = this.drawText(unitText, wx, wy, ww, 'right');
      ww -= Utils.getActualTextWidth(textSprite);
    }
    // Draw Icon
    if (iconIndex > 0) {
      if (unitText !== '') ww -= 6;
    ...
    this.resetTextColor();
    const textSprite = this.drawText(Yanfly.Util.toGroup(value), wx, wy, ww, 'right');
    ww -= Utils.getActualTextWidth(textSprite);
