...
SceneManager.updateMainWithFpsLimit = SceneManager.updateMain;

// Chosen a bit lower value than 1/60 to prevent skipping frames on insignificant slowdowns
SceneManager._fpsLimit = 1 / 62;
SceneManager._maxFrameTime = 1 / 50;

SceneManager.updateMain = function() {
    const newTime = this._getTimeInMsWithoutMobileSafari();
    const frameTime = (newTime - this._currentTime) / 1000;
    if (ConfigManager.synchFps && !STEAMDECK_MODE) {
        this._accumulator += frameTime;
        if (this._accumulator >= this._fpsLimit) {
            const remainingTime = this._accumulator % this._fpsLimit;
            this._deltaTime = Math.min(this._maxFrameTime, this._accumulator - remainingTime);
            this._accumulator = remainingTime;
        ...
        this._deltaTime = Math.min(this._maxFrameTime, frameTime);
