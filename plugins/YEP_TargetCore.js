...
DataManager.makeTargetTypes = function(obj, type) {
	...
	} else if (type.match(/REM_EVERYONE_ELSE/i)) {
      obj.scope = 'REM_EVERYONE_ELSE';
	} else if (type.match(/REM_EVERYONE/i)) {
      obj.scope = 'REM_EVERYONE';	 
...
Game_Action.prototype.isForOpponent = function() {
	...
	if (this.item().scope === 'REM_EVERYONE_ELSE') return true;
	if (this.item().scope === 'REM_EVERYONE') return true;
	if (this.item().scope === 'REM_FEMALE_ENEMY') return true;
...
Game_Action.prototype.isForEverybody = function() {
	...
	if (this.item().scope === 'REM_EVERYONE_ELSE' || this.item().scope === 'REM_EVERYONE') {
		return true;
	}
...
Game_Action.prototype.needsSelection = function() {
    ...
    if (this.item().scope === 'ENEMY ROW') return true;
    if (this.item().scope === 'ALLY ROW') return true;
	if (this.item().scope === 'REM_FEMALE_ALLY') return true;
	if (this.item().scope === 'REM_FEMALE_ENEMY') return true;
...
Game_Action.prototype.makeCustomTargets = function() {
  ...
  if (this.isForEval()) {
	...
	if(this.item().scope === 'REM_EVERYONE_ELSE' || this.item().scope === 'REM_EVERYONE') {
		if(this.item().scope === 'REM_EVERYONE') targets.push(this.subject());
		
		var length = this.opponentsUnit().aliveMembers().length;
		for (var i = 0; i < length; ++i) {
			var member = this.opponentsUnit().aliveMembers()[i];
			targets.push(member);
		}
		
		length = this.friendsUnit().members().length;
		for (var i = 0; i < length; ++i) {
			var member = this.friendsUnit().members()[i];
...
Window_Help.prototype.drawSpecialSelectionText = function(action) {
  if (action.item().customTargetText !== '') {
    ...
    var wx = 0;
    var wy = (this.contents.height - this.lineHeight()) / 2;
    var text = TextManager.yanflyTargetEverybody;
    this.drawText(text, wx, wy, this.contents.width, 'center');
...
Window_Help.prototype.makeTargetMultipleOfText = function(action) {
    ...
    if (action.isForEverybody()) {
      var targets = TextManager.yanflyTargetMultiEverybody;
    } else if (action.isForFriend()) {
      var targets = TextManager.yanflyTargetMultiAllies;
    } else {
      var targets = TextManager.yanflyTargetMultiFoes;
    }
