...
Sprite_StateIcon.prototype.initMembers = function() {
    ...
    this._turnCounterSprite.name = 'Turns';
...
Sprite_StateIcon.prototype.drawStateTurns = function(state) {
  ...
  const textStyle = contents.getTextStyle();
  textStyle.fontSize = state.turnFontSize;
  textStyle.fill = this.textColor(state.turnColor);
  textStyle.align = state.turnAlign;

  const textSprite = this._createTextSprite(turns, textStyle, wx, wy, ww, wh);
  textSprite.alpha = contents.context.globalAlpha;
  this._turnCounterSprite.addChid(textSprite);
};
...
Sprite_StateIcon.prototype.drawStateCounter = function(state) {
  ...
  textStyle.fill = this.textColor(settings.color);
  textStyle.align = settings.align;

  const textSprite = this._createTextSprite(value, textStyle, wx, wy, ww, wh);
  textSprite.alpha = contents.context.globalAlpha;
  this._turnCounterSprite.addChid(textSprite);
};
...
Sprite_StateIcon.prototype.drawBuffTurns = function(paramId) {
  ...
  const textStyle = contents.getTextStyle();
  textStyle.fontSize = Yanfly.Param.BSCFontSize;
  textStyle.fill = this.textColor(
      this._battler.isBuffAffected(paramId)
          ? Yanfly.Param.BSCBuffColor
          : Yanfly.Param.BSCDebuffColor
  );
  textStyle.align = Yanfly.Param.BSCTurnAlign;
...
Sprite_StateIcon.prototype.drawBuffRate = function(paramId) {
  ...
  const textStyle = contents.getTextStyle();
  textStyle.fontSize = Yanfly.Param.BSCFontSize * 0.75;
  textStyle.fill = this.textColor(0);
  textStyle.align = 'center';

  const textSprite = this._createTextSprite(text, textStyle, wx, wy, ww, wh);
  textSprite.alpha = contents.context.globalAlpha;
  this._turnCounterSprite.addChid(textSprite);
};
