...
Game_Battler.prototype.aoeY = function() {
    return this.spritePosY() - this.spriteHeight()/2 + this.aoeBufferY();
};
...
Game_Enemy.prototype.aoeBufferX = function() {
	let addBufferX = 0;
	if(this._hasSelectionVariables) addBufferX += this._selectionAoeBufferX;
    return this.enemy().aoeBufferX + addBufferX;
};

Game_Enemy.prototype.aoeBufferY = function() {
	let addBufferY = 0;
	if(this._hasSelectionVariables) addBufferY += this._selectionAoeBufferY;
    return this.enemy().aoeBufferY + addBufferY;
};
...
Game_Action.prototype.aoeRect = function(main) {
    ...
    if (skill.aoeRectAll) {
      ...
      rect.x = 0;
      rect.y = 0;
	} else if (skill.aoeRectColumn > 0 && skill.aoeRectRow > 0) {
...
Game_Action.prototype.isInsideAoeRect = function(r1, target) {
	...
	r2.y = target.spritePosY() - target.aoeHeight();
...
Sprite_AoeCircle.prototype.updateOpacity = function() {
    this.opacity += this._glowRate * 60 * SceneManager._deltaTime;
    if (this.opacity >= 255) {
      this._glowRate = -8;
...
Sprite_AoeRect.prototype.setup = function(skill) {
    this._skill = skill;
    if (DataManager.isAoeForbidden(skill)) return;
	if (this._skill.aoeRectColumn > 0 && this._skill.aoeRectRow > 0) {
      this._widthPixels = this._skill.aoeRectColumn;
      this._heightPixels = this._skill.aoeRectRow;
    } else if (this._skill.aoeRectColumn > 0) {
      this._widthPixels = this._skill.aoeRectColumn;
...
Sprite_AoeRect.prototype.updateLocation = function() {
	...
	let target = false;
    if (scene._enemyWindow && scene._enemyWindow.active) {
      target = scene._enemyWindow.enemy();
    } else if (scene._actorWindow && scene._actorWindow.active) {
      target = scene._actorWindow.actor();
    } else {
      return;
    }
	if(!target) return;
	if(target._hasSelectionVariables && target._selectionAoeRow > 0 && target._selectionAoeColumn > 0) {
		this.x = target.aoeX();
		this.y = target.aoeY();
	}
	else if (this._skill.aoeRectColumn > 0 && this._skill.aoeRectRow > 0) {
		this.x = target.aoeX();
		this.y = target.aoeY();
    } else if (this._skill.aoeRectColumn > 0) {
      this.x = target.aoeX();
